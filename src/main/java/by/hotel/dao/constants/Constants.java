...
public class Constants {
    ...
    public static final String GET_ALL_USERS = "SELECT `user`.`id`, `passportNumber`, `name`, `surname`, `mobilePhone`, `login`, `password`, `role_id`, `email`,`role`.`nameRole`,`role`.`update`,`role`.`delete`,`role`.`insert`,`role`.`create`,`role`.`select`,`role`.`drop`,`role`.`grant` " +
            "FROM (`db_hotel`.`user` LEFT OUTER JOIN `db_hotel`.`role` ON `user`.`role_id` = `role`.`id`)";
    public static final String ADD_USER = "INSERT INTO `db_hotel`.`user` (`passportNumber`, `name`, `surname`, `mobilePhone`, `password`, `login`,`role_id`,`email`) VALUES (?,?,?,?,?,?,?,?)";
    ...
    public static final String UPDATE_USER = "UPDATE `db_hotel`.`user` SET `passportNumber`=?, `name`=?, `surname`=?, `mobilePhone`=?, `password`=?, `login`=?, `role_id`=?, `email`=? WHERE `id`=?";
